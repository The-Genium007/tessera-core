...
fn main() {
    ...
    let client: GnsSocket<IsClient> = GnsSocket::new(g)
    ...
    while Instant::now() < deadline && !got_snapshot {
        ...
        if connected && !join_sent {
            ...
            let msg = g
                .utils()
                .allocate_message(client.connection(), SendFlags::RELIABLE, b.finished_data().to_vec());
        ...
        for m in client.receive_messages::<64>().expect("receive") {
            if let Ok(env) = flatbuffers::root::<ServerEnvelope>(m.payload()) {
                if let Some(snap) = env.msg_as_snapshot() {
                    ...
                    println!("✅ SNAPSHOT reçu — tick {}, {} autre(s) joueur(s)", snap.tick(), others);
                }
            }
        }
    ...
    if connected && got_snapshot {
        println!("🎉 PREUVE ABSOLUE : un vrai client a établi une connexion ET échangé le protocole");
        ...
        println!("❌ Aucune connexion établie (timeout). Le port répond mais pas le protocole GNS ?");
        std::process::exit(1);
    }
}
