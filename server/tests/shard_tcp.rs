...
fn client_position(x: f32) -> Vec<u8> {
    ...
    let pu = PositionUpdate::create(&mut b, &PositionUpdateArgs { position: Some(&pos), yaw: 0.0 });
    let env = ClientEnvelope::create(
        &mut b,
        &ClientEnvelopeArgs { msg_type: ClientMsg::PositionUpdate, msg: Some(pu.as_union_value()) },
    );
    b.finish(env, None);
...
async fn shard_relays_snapshots_over_tcp() {
    ...
    tokio::spawn(async move { server::shard_main(addr).await.unwrap() });
    tokio::time::sleep(Duration::from_millis(200)).await;

    let mut sock = TcpStream::connect(addr).await.unwrap();

    // Gateway → Shard : client 1 et 2 connectés, client 1 bouge en x=5.
    sock.write_all(&encode_client_event(EventKind::Connected, 1, &[])).await.unwrap();
    sock.write_all(&encode_client_event(EventKind::Connected, 2, &[])).await.unwrap();
    sock.write_all(&encode_client_event(EventKind::Message, 1, &client_position(5.0))).await.unwrap();
    ...
    loop {
        assert!(tokio::time::Instant::now() < deadline, "timeout : pas de snapshot attendu");
        ...
        while let Some(body) = reader.next_frame() {
            ...
            if let Some(players) = snap.players() {
                if players.len() == 1 && players.get(0).id() == 1 && players.get(0).position().unwrap().x() == 5.0 {
                    return; // succès
                }
            }
        }
